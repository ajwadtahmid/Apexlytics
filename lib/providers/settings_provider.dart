...
///
/// `==`/`hashCode`/`copyWith` are generated by freezed instead of
/// hand-written, so adding a field only means adding it to the constructor
/// below — run `dart run build_runner build` to regenerate
/// `settings_provider.freezed.dart`.
@freezed
abstract class PlayerSettings with _$PlayerSettings {
  // Private unnamed constructor — required by freezed whenever the class
  // also declares its own members (the getters below) alongside the
  ...
  const factory PlayerSettings({
    ...
    @Default(false) bool notifyMixtapeMapRotation,
    @Default(false) bool notifyWildcardMapRotation,
    @Default(0)
    int rankedNotifyMinutesBefore, // 0 = off, else minutes before rotation
    @Default(0) int pubsNotifyMinutesBefore,
    @Default(0) int mixtapeNotifyMinutesBefore,
    @Default(0) int wildcardNotifyMinutesBefore,
    @Default(0) int defaultTab, // 0=Home 1=Stats 2=Search 3=Settings
    @Default([]) List<String> favoriteRankedMapNames,
    @Default([]) List<String> favoritePubsMapNames,
  }) = _PlayerSettings;
